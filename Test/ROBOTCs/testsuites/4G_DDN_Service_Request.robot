...
*** Test Cases ***
TC1: LTE 4G DDN Service Request
    [Documentation]    DDN Service Request
    [Tags]    TC1    LTE_4G_DDN_Service_Request    Attach    Detach
    ...
    ${num_of_ddn_ack_sent}    ${found}    Get Key Value From Dict    ${statsTypes}    ddn_ack_sent
    ${ddnAckSentCount}    Get GRPC Stats Response Count    ${procStat}    ${num_of_ddn_ack_sent}
    ${ddnAckSentCount}    Convert to Integer    ${ddnAckSentCount}
    ...
    ${delSessionRespCount}    Get GRPC Stats Response Count    ${procStat}    ${num_of_processed_del_session_resp}
    ${delSessionRespCount}    Convert to Integer    ${delSessionRespCount}
    ...
    ${mBRespCount}    Get GRPC Stats Response Count    ${procStatAfterAttach}    ${num_of_processed_mb_resp}
    ${mBRespCount}    Convert to Integer    ${mBRespCount}
    ${incrementMbRespCount}    Evaluate    ${mbRespCountBeforeAttach}+1
    Should Be Equal    ${incrementMbRespCount}    ${mBRespCount}    Expected Modify Bearer Response Count: ${incrementMbRespCount}, but Received Modify Bearer Response Count: ${mBRespCount}    values=False
    ...
    Should Be Equal    ${incrementUeCount}    ${ueCountAfterAttach}    Expected UE Attach Count: ${incrementUeCount}, but Received UE Count: ${ueCountAfterAttach}    values=False
    ...
    ${procStatAuthResp}    ${stderr}    Execute Command    export LD_LIBRARY_PATH=${openMmeLibPath} && ${mmeGrpcClientPath}/mme-grpc-client mme-app show procedure-stats    timeout=30s    return_stderr=True
    Log    ${procStatAuthResp}
    ${authResponseCountAf}    Get GRPC Stats Response Count    ${procStatAuthResp}    ${num_of_processed_auth_response}
    ...
    ${delSessionRespCountAfDetach}    Get GRPC Stats Response Count    ${procStatOutAfDetach}    ${num_of_processed_del_session_resp}
    ${delSessionRespCountAfDetach}    Convert to Integer    ${delSessionRespCountAfDetach}
    ${incrementdelSessionRespCount}    Evaluate    ${delSessionRespCount}+1
    Should Be Equal    ${incrementdelSessionRespCount}    ${delSessionRespCountAfDetach}    Expected Delete Session Response Count: ${incrementdelSessionRespCount}, but Received Delete Session Response Count: ${delSessionRespCountAfDetach}    values=False
    ${ddnAckSentCountAfDetach}    Get GRPC Stats Response Count    ${procStatOutAfDetach}    ${num_of_ddn_ack_sent}
    ${ddnAckSentCountAfDetach}    Convert to Integer    ${ddnAckSentCountAfDetach}
    ${incrementDdnAckCount}    Evaluate    ${ddnAckSentCount}+1
    Should Be Equal    ${incrementDdnAckCount}    ${ddnAckSentCountAfDetach}    Expected UE Attach Count: ${incrementDdnAckCount}, but Received UE Attach Count: ${ddnAckSentCountAfDetach}    values=False
