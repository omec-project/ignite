...
*** Test Cases ***
TC1: LTE 4G S1 Release
    [Documentation]    S1 Release
    [Tags]    TC1    LTE_4G_S1_Release    Attach    Detach
    ...
    Log    ${procStat}
    ${num_of_rel_access_bearer_req_sent}    ${found}    Get Key Value From Dict    ${statsTypes}    rel_access_bearer_req_sent
    ${noOfRelBearRespRecvBefAttach}    Get GRPC Stats Response Count    ${procStat}    ${num_of_rel_access_bearer_req_sent}
    ${num_of_s1_rel_cmd_sent}    ${found}    Get Key Value From Dict    ${statsTypes}    s1_rel_cmd_sent
    ${noOfS1RelRecvBefAttach}    Get GRPC Stats Response Count    ${procStat}    ${num_of_s1_rel_cmd_sent}
    ...
    Send S1ap    ue_context_release_cmp    ${ueContextReleaseCmp}    ${enbUeS1APId}    #eNB sends UE Context Release Complete to MME
    Sleep    1s
    ${procStatOutAfterS1RelDetach}    Execute Command    export LD_LIBRARY_PATH=${openMmeLibPath} && ${mmeGrpcClientPath}/mme-grpc-client mme-app show procedure-stats    timeout=30s
    Log    ${procStatOutAfterS1RelDetach}
    ${ueCountAfterS1RelDetach}    Get GRPC Stats Response Count    ${procStatOutAfterS1RelDetach}    ${num_of_subscribers_attached}
    ${ueCountAfterS1RelDetach}    Convert to Integer    ${ueCountAfterS1RelDetach}
    Should Be Equal    ${ueCountBeforeAttach}    ${ueCountAfterS1RelDetach}    Expected UE Detach Count: ${ueCountBeforeAttach}, but Received UE Detach Count: ${ueCountAfterS1RelDetach}    values=False
    ${noOfRelBearRespRecvAfterS1Rel}    Get GRPC Stats Response Count    ${procStatOutAfterS1RelDetach}    ${num_of_rel_access_bearer_req_sent}
    ${noOfRelBearRespRecvAfterS1Rel}    Convert to Integer    ${noOfRelBearRespRecvAfterS1Rel}
    ${noOfS1RelRecvAfterS1Rel}    Get GRPC Stats Response Count    ${procStatOutAfterS1RelDetach}    ${num_of_s1_rel_cmd_sent}
    ${noOfS1RelRecvAfterS1Rel}    Convert to Integer    ${noOfS1RelRecvAfterS1Rel}
    ${noOfS1RelRecvBefAttach}    Evaluate    ${noOfS1RelRecvBefAttach}+1
    ${noOfRelBearRespRecvBefAttach}    Evaluate    ${noOfRelBearRespRecvBefAttach}+1
    Should Be Equal    ${noOfRelBearRespRecvBefAttach}    ${noOfRelBearRespRecvAfterS1Rel}    Expected Release Bearer Response Sent Count: ${noOfRelBearRespRecvBefAttach}, but Received Release Bearer Response Sent Count: ${noOfRelBearRespRecvAfterS1Rel}    values=False
    Should Be Equal    ${noOfS1RelRecvBefAttach}    ${noOfS1RelRecvAfterS1Rel}    Expected S1 Release Sent Count: ${noOfS1RelRecvBefAttach}, but Received S1 Release Sent Count: ${noOfS1RelRecvAfterS1Rel}    values=False
