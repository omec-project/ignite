...
TC1: LTE 4G Cancel Location
    ...
    ${num_of_cla_sent}    ${found}    Get Key Value From Dict    ${statsTypes}    cla_sent
    ${numCLASend}    Get GRPC Stats Response Count    ${procStatOutBfExec}    ${num_of_cla_sent}
    ${numCLASend}    Convert to Integer    ${numCLASend}
    ${num_of_clr_received}    ${found}    Get Key Value From Dict    ${statsTypes}    clr_received
    ${numCLRReceived}    Get GRPC Stats Response Count    ${procStatOutBfExec}    ${num_of_clr_received}
    ${numCLRReceived}    Convert to Integer    ${numCLRReceived}
    ...
    ${numCLRReceivedAf}    Get GRPC Stats Response Count    ${procStatOutAfDetach}    ${num_of_clr_received}
    ${numCLRReceivedAf}    Convert to Integer    ${numCLRReceivedAf}
    ${incrementclrCount}    Evaluate    ${numCLRReceived}+1
    Should Be Equal    ${incrementclrCount}    ${numCLRReceivedAf}    Expected num of clr received Count: ${incrementclrCount}, but Received num of clr received Count: ${numCLRReceivedAf}    values=False
    ${numCLASendAf}    Get GRPC Stats Response Count    ${procStatOutAfDetach}    ${num_of_cla_sent}
    ${numCLASendAf}    Convert to Integer    ${numCLASendAf}
    ${incrementClaCount}    Evaluate    ${numCLASend}+1
    Should Be Equal    ${incrementClaCount}    ${numCLASendAf}    Expected num of cla received Count: ${incrementClaCount}, but Received num of cla received Count: ${numCLASendAf}    values=False
