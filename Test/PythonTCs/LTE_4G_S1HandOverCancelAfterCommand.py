...
try:
    ...
    num_of_processed_ho_cancel = int(do.splitProcStats(proc_stat, stats_type["ho_cancel"]))
    ...
    igniteLogger.logger.info("\n---------------------------------------\nHandover cancel  sent from source eNB to MME\n---------------------------------------")
    s1.sendS1ap('handover_cancel', handover_cancel, enbues1ap_id)

    igniteLogger.logger.info("\n---------------------------------------\nTarget eNB receives UE Context Release Request from MME\n---------------------------------------")
    s1.receiveS1ap(target=True)

    igniteLogger.logger.info("\n---------------------------------------\nHandover Cancel Acknowledge  received from MME to source eNB\n---------------------------------------")
    s1.receiveS1ap()
    ...
    num_of_processed_ho_cancel_Afdetach = int(do.splitProcStats(proc_stat_af_detach, stats_type["ho_cancel"]))
    icu.grpcValidation(num_of_processed_ho_cancel + 1, num_of_processed_ho_cancel_Afdetach, "Number of Handover Cancel")
