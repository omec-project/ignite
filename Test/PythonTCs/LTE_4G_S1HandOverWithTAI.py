...
try:
    ...
    num_of_processed_ho_required = int(do.splitProcStats(proc_stat, stats_type["ho_required"]))
    num_of_processed_ho_request_ack = int(do.splitProcStats(proc_stat, stats_type["ho_request_ack"]))
    num_of_processed_ho_notify = int(do.splitProcStats(proc_stat, stats_type["ho_notify"]))
    num_of_processed_purge_resp = int(do.splitProcStats(proc_stat, stats_type["purge_resp"]))

    print ("\n-------------------------------------\nIMSI Attach-HO-TAI-Detach Execution Started\n---------------------------------------")
    ...
    igniteLogger.logger.info("\n---------------------------------------\nHandover Required sent from eNB to MME\n---------------------------------------")
    icu.updateKeyValueInDict(handover_required,"tAC","1002")
    s1.sendS1ap('handover_required', handover_required, enbues1ap_id)
    
    igniteLogger.logger.info("\n---------------------------------------\nHandover Request received from MME to target eNB\n---------------------------------------")
    s1.receiveS1ap(target=True)
    
    igniteLogger.logger.info("\n---------------------------------------\nHandover Request Acknowledge sent from target eNB to MME\n---------------------------------------")
    s1.sendS1ap('handover_request_acknowledge', handover_request_acknowledge, enbues1ap_id)
    
    igniteLogger.logger.info("\n---------------------------------------\nHandover Command message received from MME to eNB\n---------------------------------------")
    s1.receiveS1ap()   
   
    igniteLogger.logger.info("\n---------------------------------------\nHandover Notify message sent from target eNB to MME\n---------------------------------------")
    icu.updateKeyValueInDict(handover_notify,"tAC","1002")
    s1.sendS1ap('handover_notify', handover_notify, enbues1ap_id)  
    ...
    num_of_processed_ho_required_Afdetach = int(do.splitProcStats(proc_stat_af_detach, stats_type["ho_required"]))
    icu.grpcValidation(num_of_processed_ho_required + 1, num_of_processed_ho_required_Afdetach, "Number of Handover Required")

    num_of_processed_ho_request_ack_Afdetach = int(do.splitProcStats(proc_stat_af_detach, stats_type["ho_request_ack"]))
    icu.grpcValidation(num_of_processed_ho_request_ack + 1, num_of_processed_ho_request_ack_Afdetach, "Number of Handover Request Acknowledge")

    num_of_processed_ho_notify_Afdetach = int(do.splitProcStats(proc_stat_af_detach, stats_type["ho_notify"]))
    icu.grpcValidation(num_of_processed_ho_notify + 1, num_of_processed_ho_notify_Afdetach, "Number of Handover Notify")
    ...
    print ("\n-------------------------------------\nIMSI Attach-HO-TAI-Detach Execution Successful\n---------------------------------------")
