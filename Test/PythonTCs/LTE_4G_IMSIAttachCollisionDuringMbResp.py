...
try:
    ...
    print ("\n-------------------------------------\nIMSI Attach Collision during Modify Bearer Response - Execution Started\n---------------------------------------")
    ...
    icu.grpcValidation(num_of_processed_aia + 2, num_of_processed_aia_afattach, "Number of Processed AIA")

    num_of_processed_ula_afattach = int(do.splitProcStats(proc_stat_af_attach, stats_type["processed_ula"]))
    icu.grpcValidation(num_of_processed_ula + 2, num_of_processed_ula_afattach, "Number of Processed ULA")

    num_of_handled_esm_info_resp_afattach = int(do.splitProcStats(proc_stat_af_attach, stats_type["esm_info_resp"]))
    icu.grpcValidation(num_of_handled_esm_info_resp + 2, num_of_handled_esm_info_resp_afattach,
    ...
    icu.grpcValidation(num_of_processed_sec_mode_resp + 2, num_of_processed_sec_mode_resp_afattach,
    ...
    icu.grpcValidation(num_of_processed_init_ctxt_resp + 2, num_of_processed_init_ctxt_resp_afattach,
    ...
    print ("\n-------------------------------------\nIMSI Attach Collision during Modify Bearer Response - Execution Successful\n---------------------------------------")
